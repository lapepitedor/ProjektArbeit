...
import { Firestore, addDoc, collection, getDocs, query, deleteDoc, updateDoc, getDoc, doc, where } from '@angular/fire/firestore';
...
export class DashboardComponent implements OnInit
{
        ...
        private loginService: LoginService, private snackBar: MatSnackBar, private router: Router)
    {
        ...
        {
            // console.log("🚀 ~ DashboardComponent ~ category:", category)
    ...
    getUserExpenses()
    {
        const currentUser: any = this.loginService.getUser() ? this.loginService.getUser()?.email : null;
    ...
    async getExpenses()
    {
        ...
        {
            const q = query(collection(this.afs.firestore, `users/${this.userId}/expenses`), where("dateinMili", ">", this.startDate.getTime()), where("dateinMili", "<", this.endDate.getTime()));
    ...
    filterData(snapshots: any)
    {
        ...
        const firstDate = new Date(Math.min.apply(null, parsedData.map((e: any) => new Date(e.date))));  
        const lastDate = new Date(Math.max.apply(null, parsedData.map((e: any) => new Date(e.date))));    

        const numOfEntries = parsedData.length;
        const getTotal = this.getTotal(parsedData);
        const totalAmount = getTotal.categorySum;
        // this.balanceAmt = getTotal.balanceAmt;
        // this.incomeAmt = getTotal.incomeAmt;
        // this.expenseAmt = getTotal.expenseAmt;
        ...
        this.metrics = [
            { color: 'money-icon', value: firstDate.toDateString().slice(3, 15), metricTitle: 'First Expense Date', icon: 'today' },
            { color: 'money-icon', value: lastDate.toDateString().slice(3, 15), metricTitle: 'Latest Expense Date', icon: 'today' },
            { color: 'money-icon', value: numOfEntries, metricTitle: 'Number of Expenses', icon: 'receipt' },
        ...
        // console.log("🚀 ~ DashboardComponent ~ filterData ~ pieData:", pieData)
